export default [
    {
        "number": 1,
        "title": "Harry Potter e a Pedra Filosofal",
        "originalTitle": "Harry Potter and the Sorcerer's Stone",
        "releaseDate": "Jun 26, 1997",
        "description": "No seu aniversário, Harry Potter descobre que é filho de dois bruxos conhecidos, de quem herdou poderes mágicos. Ele deve frequentar uma famosa escola de magia e feitiçaria, onde estabelece amizade com dois jovens que se tornarão seus companheiros em sua aventura. Durante seu primeiro ano em Hogwarts, ele descobre que um bruxo malévolo e poderoso chamado Voldemort está em busca de uma pedra filosofal que prolonga a vida de seu dono.",
        "pages": 223,
        "cover": "https://raw.githubusercontent.com/fedeperin/potterapi/main/public/images/covers/1.png"
    },
    {
        "number": 2,
        "title": "Harry Potter e a Câmara Secreta",
        "originalTitle": "Harry Potter and the chamber of secrets",
        "releaseDate": "Jul 2, 1998",
        "description": "Harry Potter e os alunos do segundo ano investigam uma ameaça malévola aos seus colegas de Hogwarts, uma fera ameaçadora que se esconde dentro do castelo.",
        "pages": 251,
        "cover": "https://raw.githubusercontent.com/fedeperin/potterapi/main/public/images/covers/2.png"
    },
    {
        "number": 3,
        "title": "Harry Potter e o Prisioneiro de Azkaban",
        "originalTitle": "Harry Potter and the Prisoner of Azkaban",
        "releseDay": "Jul 8, 1999",
        "description": "O terceiro ano de estudos de Harry em Hogwarts é ameaçado pela fuga de Sirius Black da prisão de Azkaban. Aparentemente, trata-se de um bruxo perigoso que foi cúmplice de Lord Voldemort e que tentará se vingar de Harry Potter.",
        "pages": 317,
        "cover": "https://raw.githubusercontent.com/fedeperin/potterapi/main/public/images/covers/3.png"
    },
    {
        "number": 4,
        "title": "Harry Potter e o Cálice de Fogo",
        "originalTitle": "Harry Potter and the Goblet of Fire",
        "releaseDate": "Jul 8, 2000",
        "description": "Hogwarts se prepara para o Torneio Tribruxo, no qual competirão três escolas de magia. Para surpresa de todos, Harry Potter é o escolhido para participar da competição, na qual deverá lutar contra dragões, entrar na água e enfrentar seus maiores medos.",
        "pages": 636,
        "cover": "https://raw.githubusercontent.com/fedeperin/potterapi/main/public/images/covers/4.png"
    },
    {
        "number": 5,
        "title": "Harry Potter e a Ordem da Fênix",
        "originalTitle": "Harry Potter and the Order of the Phoenix",
        "releaseDate": "Jun 21, 2003",
        "description": "Em seu quinto ano em Hogwarts, Harry descobre que muitos membros da comunidade bruxa não sabem a verdade sobre seu encontro com Lord Voldemort. Cornelius Fudge, Ministro da Magia, nomeia Dolores Umbridge como professora de Defesa Contra as Artes das Trevas porque acredita que o Professor Dumbledore planeja assumir seu cargo. Mas seus ensinamentos são inadequados, então Harry prepara os alunos para defenderem a escola contra o mal.",
        "pages": 766,
        "cover": "https://raw.githubusercontent.com/fedeperin/potterapi/main/public/images/covers/5.png"
    },
    {
        "number": 6,
        "title": "Harry Potter e o Enigma do Príncipe",
        "originalTitle": "Harry Potter and the Half-Blood Prince",
        "releaseDate": "Jul 16, 2005",
        "description": "Harry descobre um livro poderoso e, ao mesmo tempo em que tenta descobrir suas origens, colabora com Dumbledore na busca por uma série de objetos mágicos que ajudarão na destruição de Lord Voldemort.",
        "pages": 607,
        "cover": "https://raw.githubusercontent.com/fedeperin/potterapi/main/public/images/covers/6.png"
    },
    {
        "number": 7,
        "title": "Harry Potter e as Relíquias da Morte",
        "originalTitle": "Harry Potter and the Deathly Hallows",
        "releaseDate": "Jul 21, 2007",
        "description": "Harry, Ron e Hermione partem em uma missão perigosa para localizar e destruir o segredo da imortalidade e destruição de Voldemort - as Horcruces. Sozinhos, sem a orientação de seus professores ou a proteção do Professor Dumbledore, os três amigos devem contar uns com os outros mais do que nunca. Mas existem Forças das Trevas que ameaçam separá-los. Harry Potter está cada vez mais perto da tarefa para a qual se prepara desde o primeiro dia em que pisou em Hogwarts: a última batalha com Voldemort.",
        "pages": 607,
        "cover": "https://raw.githubusercontent.com/fedeperin/potterapi/main/public/images/covers/7.png"
    },
    {
        "number": 8,
        "title": "Harry Potter e a Criança Amaldiçoada",
        "originalTitle": "Harry Potter and the Cursed Child",
        "releaseDate": "Jul 30, 2016",
        "description": "O segundo filho de Harry entrou em Hogwarts, mas na Sonserina. Seu relacionamento com Harry está piorando e ele se tornou amigo íntimo do filho de Draco, Scorpius Malfoy, que dizem ser filho de Lord Voldemort.",
        "pages": 336,
        "cover": "https://raw.githubusercontent.com/fedeperin/potterapi/main/public/images/covers/8.png"
    }
]